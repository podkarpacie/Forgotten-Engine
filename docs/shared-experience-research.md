...
## Atomic award design requirement

The current static-defeat reward helper is deliberately single-player: it mutates the authoritative world and then uses one-player persistence methods. It cannot be reused for a shared award because a later database failure could expose an in-memory result to only some party recipients. The required next boundary is therefore a cloned/staged `WorldState`, an immutable outcome list ordered by recipient ID, one SQLite transaction that validates and writes every resulting player row, and only then replacement of the synchronized live state plus the relevant refresh epochs. When shared eligibility is absent, the existing one-player path must remain unchanged.
