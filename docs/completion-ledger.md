...
| Native profile session and protocol compatibility | 12% | 3% | The experimental profile-driven native OTCv8 740 path covers login, character selection, map delivery, movement, visibility, parser-safe chat input, bounded internal chat fanout, and partial state refresh. Client-visible public chat is intentionally suppressed because the audited 740 client has no message-mode map. Full protocol coverage and stable client evidence are incomplete. |
