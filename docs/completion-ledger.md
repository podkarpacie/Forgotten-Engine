...
- [x] Activate exactly one imported teleport destination after a successful native cardinal or diagonal move lands on its tile; revalidate and persist the final relocation through the existing full viewport refresh, while retaining ordinary movement for missing, ambiguous, or blocked triggers. Chains, effects, scripts, and general tile behavior remain deferred.
