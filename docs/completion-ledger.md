...
- [x] Retain bounded legacy `items.xml` armor, optional unsigned weight, defense, extra-defense, attack-speed, and verified weapon-type metadata for known OTB items. Positive armor is now a narrowly scoped immutable input for the declarative native weapon bridge; weight and all remaining fields are non-executing metadata, and no formula or capacity compatibility is claimed.
