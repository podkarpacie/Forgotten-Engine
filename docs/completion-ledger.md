...
- [x] Expose existing complete-item equipment/container transactions through the shared world and advance both native refresh epochs only after core success; persistence and client routing remain deferred except for the tested native 740 whole-item fixed-equipment-to-owned-top-level-container route.
...
The next implementation slice is **native 740 top-level-container-to-equipment transfer**, followed by **native 740 temple-respawn delivery**. The completed equipment-to-container route has authoritative persistence and parser-verified socket coverage, but it still lacks lawful real-client evidence. Neither slice claims generic inventory, full lifecycle delivery, formula, spell, PvP, or TFS parity.
