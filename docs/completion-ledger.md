...
- [x] Append one bounded imported OTBM description to that same validated native 740 LookMap status frame only when it remains within the existing text-message bound; a valid imported source weight sentence precedes it when available. Missing, mismatched, empty, or oversized metadata retains the preceding bounded text. Generated names, articles, attributes, recursive container weight, capacity, text contents, and broader inspection behavior remain deferred.
