...
| Map item use | partial — typed server-side validation accepts only same-tile or adjacent use of an exact top-level authoritative map item stack and returns immutable metadata without mutation; action execution, doors, switches, containers, charges, Lua, and client item-use packets remain deferred | deferred | deferred |
