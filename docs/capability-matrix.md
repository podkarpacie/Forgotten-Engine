...
| Combat | partial — typed physical adjacent-melee events with bounded damage, deterministic per-player server-tick cooldowns, profile-neutral flat physical mitigation, and an optional scriptless FE declarative weapon catalog that can construct typed events; persisted vitals and native selected-player health updates remain available. The current native path supplies only a fixed 10 HP event with default zero mitigation. Equipped-item binding, TFS Lua weapon scripts, legacy XML formula semantics, armor, shielding, spells, ranged delivery, resistance, PvP, persistence, automatic attacks, and client combat effects remain deferred | deferred | deferred |
