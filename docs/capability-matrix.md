...
| Runtime items, inventory, containers, equipment, depot, inbox | partial — bounded core item/equipment/container primitives, authoritative player equipment state, SQLite equipment persistence, native-session hydration, parser-verified catalog-mapped 740 equipment bootstrap, and a non-emitting parser-verified container-open codec; no incremental inventory synchronization, container persistence or nesting, live container-window delivery, or use semantics | deferred | deferred |
