...
| Typed skill progression, experience and vocation rules | partial — typed seven-skill levels/percentages and numeric vocation identity are authoritative, SQLite-persisted, locally provisionable, and delivered in classic 740 bootstrap/refresh records; operator-owned TFS-style `data/XML/vocations.xml` supplies profile-gated online health/mana regeneration, while bounded `data/XML/stages.xml` plus `rateExp` can drive a deterministic authoritative operator experience award with SQLite persistence; hydrated poison/burning/energy schedules update native vitals; combat, quest, monster, and client reward event sources, condition effect packets, condition-driven death activation, runtime condition-tick persistence, soul, death/respawn, skill tries/formulas, promotion runtime, and full TFS-compatible experience calculation remain deferred | deferred | deferred |
