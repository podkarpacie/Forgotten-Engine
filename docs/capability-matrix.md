...
| Runtime items, inventory, containers, equipment, depot, inbox | partial — bounded core item/equipment/container primitives, authoritative player equipment/container state, SQLite equipment/container persistence, native-session hydration, and parser-verified catalog-mapped 740 equipment bootstrap in deterministic multi-slot order. Extended diagnostics report unmapped equipped items without exposing packet bodies. A non-emitting parser-verified container-open codec and an operator command for atomic complete-item transfer from one equipment slot into one already owned bounded container are present; native incremental inventory synchronization, map-ground transfer, recursive nesting, unsolicited live container windows, stack splitting, and item-use semantics remain deferred | deferred | deferred |
