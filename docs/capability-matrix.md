...
| Player outfit | partial — schema-v11 persists accepted matching classic-740 look type and color values, then hydrates them after map initialization with a configured look-type fallback for legacy or mismatched records. A native `0xD2` request receives the classic `0xC8` current-outfit plus one-byte start/end look-type range; no newer outfit-list, addon, or mount fields are emitted. Look-type selection, addons, mounts, asset validation, and unmodified-client confirmation remain deferred | deferred | deferred |
