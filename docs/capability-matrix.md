...
| Static creature rendering/occupancy | partial — catalog-derived static spawn representation, native visibility, optional deterministic movement, caller-triggered reactivation, and propagated legacy per-spawn intervals for deterministic due-reactivation at an unoccupied spawn position. A synchronized shared-world call advances the visibility epoch only after a real reactivation. Automatic global scheduling, AI, combat, loot, corpses, scripts, and real-client observation remain deferred | deferred | deferred |
