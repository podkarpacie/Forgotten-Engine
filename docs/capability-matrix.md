...
| Static creature rendering/occupancy | partial — catalog-derived static spawn representation, native visibility, optional deterministic movement, caller-triggered reactivation, propagated legacy per-spawn intervals for deterministic due-reactivation at an unoccupied spawn position, and native target selection of active authoritative static IDs. Selecting a static entity creates no combat, movement, follow, damage, loot, corpse, script, or packet side effect; deactivation clears the target. One native game-host heartbeat advances the shared-world tick and evaluates due reactivation; client sessions no longer advance shared time. The visibility epoch changes only after a real reactivation. AI, combat, loot, corpses, scripts, broader-profile coverage, end-to-end service lifecycle testing, and real-client observation remain deferred | deferred | deferred |
