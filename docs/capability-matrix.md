...
| Combat | partial — typed physical adjacent-melee events with bounded damage, deterministic per-player server-tick cooldowns, persisted vitals, and native selected-player health updates; the current native path supplies only a fixed 10 HP event. Defensive values, weapon formulas, spells, ranged delivery, resistances, PvP policy, automatic attacks, and client combat effects remain deferred | deferred | deferred |
