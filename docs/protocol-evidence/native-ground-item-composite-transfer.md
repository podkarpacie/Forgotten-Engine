...
The native game service creates one synchronized map owner before its listener thread starts. When a durable journal exists, startup loads it and calls the guarded source-recovery primitive before owner construction; an incompatible revision, missing ordered source item, or duplicate identity prevents native game startup. Each accepted session receives a detached owner snapshot, and the heartbeat obtains a detached owner snapshot per tick.
The native classic 740 route now covers one normal source-map coordinate to one empty fixed equipment slot. It requires a unique configured catalog mapping, exact source identity and range validation, and the complete source count. After the composite transaction succeeds, FE sends the existing `SetInventory` record plus one full-map (`0x64`) refresh. It does not claim a compact map-delta record. Container destinations, stack splitting and merging, capacity, occupied-slot behavior, slot compatibility, ground-item ownership beyond the source map, map actions, and real unmodified OTClientV8 confirmation remain deferred.
