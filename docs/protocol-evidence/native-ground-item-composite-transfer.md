...
The native game service creates one synchronized map owner before its listener thread starts. When a durable journal exists, startup loads it and calls the guarded source-recovery primitive before owner construction; an incompatible revision, missing ordered source item, or duplicate identity prevents native game startup. Each accepted session receives a detached owner snapshot, and the heartbeat obtains a detached owner snapshot per tick. Native classic `ThrowItem` source-position decoding must still be routed to this foundation. That future route needs exact catalog client-item validation and parser-verified inventory/map delta delivery. Container destinations, stack splitting and merging, capacity, slot compatibility, ground-item ownership rules beyond the source map, map actions, and real unmodified OTClientV8 confirmation remain deferred.
