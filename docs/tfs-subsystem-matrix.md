...
| `monster.*`, `monsters.*`, `npc.*`, `spawn.*` | Monster and NPC definitions, spawn timing, AI, target selection, combat, dialogue, shops, and movement. | **Partial.** FE materializes static spawn entities, persists bounded state, has deterministic reset/occupancy behavior, safe movement, player-selected static targets, a core-only nearest-player selector, an opt-in shared acquisition policy, and an opt-in select-and-single-step pursuit policy with legal map/occupancy fallback. A real shared-world step emits the existing native full-map refresh. | Implement deterministic creature lifecycle, default scheduling, pursuit/path boundaries, combat participation, loot/corpses, NPC behavior, and client evidence before claiming dynamic-creature support. |
