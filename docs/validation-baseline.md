...
| Vocation advancement metadata | Parse a lawful `vocations.xml` fixture and confirm bounded `gainhp`, `gainmana`, and `gaincap` values are retained as immutable configuration. Confirm experience awards do not yet alter player vitals, capacity, or HUD values from this metadata. | A dropped or misparsed value, implicit level/stat mutation, formula-parity claim, or client-visible advancement effect without its own validated runtime path is a release blocker. |
