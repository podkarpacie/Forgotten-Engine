...
| Outfit dialog and persistence | Configure an inclusive valid classic range with `otclientV8OutfitFirstLookType` and `otclientV8OutfitLastLookType`, open Set Outfit, and verify the native `0xD2` request receives one classic `0xC8` dialog containing the current appearance and exactly that range. Change accepted color values, relog the same character, and confirm the stored look is visible after map initialization. | A parser error, disconnect, missing dialog, range that excludes the current look, lost stored colors, unexpected newer-format fields, missing operator-owned assets, or acceptance of an unsupported look type is a release blocker. |
