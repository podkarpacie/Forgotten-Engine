...
| Outfit dialog and persistence | Open Set Outfit, verify the native `0xD2` request receives one classic `0xC8` dialog containing the current appearance and its bounded start/end look range, change accepted color values, relog the same character, and confirm the stored look is visible after map initialization. | A parser error, disconnect, missing dialog, lost stored colors, unexpected newer-format fields, or acceptance of an unsupported look type is a release blocker. |
...
| Static selected melee | Select an active adjacent static entity and wait for the native heartbeat. Verify each supported fixed 10-point hit refreshes its visible health; a hit that reaches zero clears the target and removes the entity from the active map view. Confirm later configured reactivation restores the imported percentage. | A parser error, disconnect, damage outside adjacent selected targets, missing visibility refresh, a stale target, client-supplied damage, loot/corpse/reward creation, AI behavior, or failure to restore configured health on reactivation is a release blocker. |
