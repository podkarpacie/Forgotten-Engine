    ...
    loop {
        ...
        match action {
            ...
            NativeOtClientGameAction::ThrowItem {
                ...
                // The catalog and count gates stay loop-level so every router below sees
                // the same preconditions in the same order; handlers re-derive the
                // catalog gate with the same diagnostic to stay total.
                if config.item_presentation_catalog.is_none() {
                    native_diagnostic(
                        config.extended_diagnostics,
                        peer,
                ...
                if source_position.x != 0xffff {
                    ...
                    // registry; imported source items keep their own transfer paths below.
                    {
                        ...
                        if apply_native_throw_item_runtime_pickup(
                            &mut ctx,
                            map_owner,
                            ...
                            &closed_container_ids,
                            &mut sent_container_windows,
                            &mut ThrowItemRuntimePickupFollow {
                                observed_mapped_equipment: &mut observed_mapped_equipment,
                                observed_equipment_epoch: &mut observed_equipment_epoch,
                                observed_containers_epoch: &mut observed_containers_epoch,
                            },
                ...
                // Equipment sources move into owned equipment or owned containers;
                // the fallthrough tail always consumes the record.
                {
                    let mut ctx = SessionContext {
                        stream: &mut *stream,
                        peer,
                    ...
                    if apply_native_throw_item_equipment_source(
                        &mut ctx,
                        ThrowItemEquipmentSourceRequest {
                            source_slot,
                            target_slot,
                            target_container_id,
                            target_position,
