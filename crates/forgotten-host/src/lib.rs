...
mod tests {
    ...
    #[test]
    fn native_live_spawn_step_in_routes_through_a_wired_dispatcher() {
        let database_path = database_path("native-live-spawn-step-in");
        let database = EngineDatabase::open(&database_path).unwrap();
        let account_id = database
            .create_account_with_password("operator", "correct horse battery staple")
            .unwrap();
        ...
        dispatcher
            .register_callback(
                "movement:stepin:0",
                "return function() doCreatureSay('spawned') end",
            )
            .unwrap();
        let mut native_config = native_empty_world_config("127.0.0.1:0".parse().unwrap());
        native_config.movement_dispatcher = Some(Arc::new(dispatcher));
        ...
        // Spawning onto the scripted tile fires StepIn with no movement at all.
        stream
            .set_read_timeout(Some(Duration::from_millis(500)))
            .unwrap();
        ...
        loop {
            match read_frame(&mut stream) {
                ...
                Ok(frame) => {
                    if String::from_utf8_lossy(&frame.0).contains("spawned") {
                        spawned = true;
                        break;
                    }
                }
                ...
                Err(error) => panic!("native session ended during spawn probe: {error}"),
            }
        }
        assert!(spawned, "spawn StepIn Say effect never arrived");
        game.shutdown().unwrap();
        let _ = fs::remove_file(database_path);
    }

    #[test]
    fn native_live_step_out_routes_through_a_wired_dispatcher() {
        let database_path = database_path("native-live-step-out");
        let database = EngineDatabase::open(&database_path).unwrap();
        let account_id = database
            .create_account_with_password("operator", "correct horse battery staple")
            .unwrap();
        ...
        drop(database);
        let registry = parse_tfs_movements_xml(
            br#"<movements><moveevent type="StepOut" itemid="2151" script="door.lua"/></movements>"#,
        )
        .unwrap();
        let mut dispatcher = SandboxedLuaCallbackDispatcher::default();
        dispatcher
            .register_callback(
                "movement:stepout:0",
                "return function() doCreatureSay('leaving') end",
            )
            .unwrap();
        let mut native_config = native_empty_world_config("127.0.0.1:0".parse().unwrap());
        native_config.movement_dispatcher = Some(Arc::new(dispatcher));
        ...
        // Step east off the scripted tile: the StepOut script answers Say for the
        // departed tile while the plain arrival tile stays silent.
        write_frame(
            &mut stream,
            &Frame(vec![forgotten_protocol::NATIVE_OTCLIENT_CLIENT_WALK_EAST]),
        )
        ...
        loop {
            match read_frame(&mut stream) {
                ...
                Ok(frame) => {
                    if String::from_utf8_lossy(&frame.0).contains("leaving") {
                        leaving = true;
                        break;
                    }
                }
    ...
    #[test]
    fn native_live_deequip_routes_through_a_wired_dispatcher() {
        let database_path = database_path("native-live-deequip");
        let mut database = EngineDatabase::open(&database_path).unwrap();
        let account_id = database
            .create_account_with_password("operator", "correct horse battery staple")
            .unwrap();
        ...
        let registry = parse_tfs_movements_xml(
            br#"<movements><moveevent type="DeEquip" itemid="4526" slot="right-hand" script="shed.lua"/></movements>"#,
        )
        .unwrap();
        let mut dispatcher = SandboxedLuaCallbackDispatcher::default();
        dispatcher
            .register_callback(
                "movement:deequip:0",
                "return function() doCreatureSay('shed') end",
            )
            .unwrap();
        let mut config = native_empty_world_config("127.0.0.1:0".parse().unwrap());
        config.item_presentation_catalog = Some(Arc::new(catalog));
        ...
        // Throw the equipped sword into the backpack: the vacated slot fires DeEquip
        // through the live equipment-tail hook.
        write_frame(
            &mut client,
            &Frame(vec![
                forgotten_protocol::NATIVE_OTCLIENT_CLIENT_THROW_ITEM,
        ...
        loop {
            match read_frame(&mut client) {
                ...
                Ok(frame) => {
                    if String::from_utf8_lossy(&frame.0).contains("shed") {
                        shed = true;
                        break;
                    }
                }
                ...
                Err(error) => panic!("native session ended during deequip probe: {error}"),
            }
        }
