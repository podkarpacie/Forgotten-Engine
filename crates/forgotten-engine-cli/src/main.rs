...
fn load_independent_native_startup_content(
    ...
    thread::scope(
        |scope| -> Result<IndependentNativeStartupContent, Box<dyn std::error::Error>> {
            ...
            let movement_dispatcher = movement_dispatcher
                .join()
            ...
            let quest_catalog = quest_catalog
                .join()
                .map_err(|_| "quest catalog loader worker panicked")??;
...
fn build_movement_dispatcher(
    config: &EngineConfig,
) -> Result<
    (
        Option<SandboxedLuaCallbackDispatcher>,
        Option<Arc<TfsMoveEventRegistry>>,
    ),
    String,
> {
    let registry = load_tfs_movement_registry(config).map_err(|error| error.to_string())?;
    if registry.is_empty() {
        return Ok((None, None));
    }
    let mut dispatcher = SandboxedLuaCallbackDispatcher::default();
...
fn run_host(
    ...
    let item_name_by_server_id = item_catalog
        .as_ref()
        .map(|catalog| catalog.xml_name_by_server_id());
