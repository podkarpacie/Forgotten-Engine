...
The matching native game listener accepts the normal pending-game selection request and verifies account ownership of the chosen character. By default it returns a normal game-login error because the world fixture is disabled. An operator can opt in to the original native empty-world fixture by setting `otclientV8NativeEmptyWorldEnabled = true` and supplying nonzero `otclientV8EmptyWorldGroundThingId` and `otclientV8PlayerLookType` values that exist in the operator’s lawful matching 740 thing data. FE then sends normal player-login (`0x0a`) and full-map (`0x64`) frames containing an 18×14, floors-0-through-7 ground fixture and one local player creature. It accepts normal cardinal movement opcodes and sends normal creature-move (`0x6d`) responses while the player remains in that initially described viewport, persisting the accepted position.

The native profile is selected from explicit `config.lua` behavior fields rather than being inferred from an FE release label, and unsupported profile/fixture combinations are rejected with an explicit configuration diagnostic. The initial fixture does not contain Forgotten Engine-distributed thing data, maps, items, collision, map-row deltas, or other creatures. Once the player reaches the described viewport boundary, FE returns an explicit normal error rather than pretending to stream a real map.

It is still **not** an official Tibia login/game service or a broadly playable normal OTClientV8 world. The older FE-aware custom-client endpoint returns only initial identity, stored position, endpoint metadata, and an empty-world gate. The native endpoint now proves normal credential selection, player initialization, full-map parsing, and bounded cardinal movement only; it does not provide real map/datapack loading, combat, scripting, or continuous world simulation.
...
| Native OTClientV8 740 empty-world path | Profile-selected normal login/pending-game decoding, numeric account authentication, legacy character-list, player-login, full-map, and cardinal move packets. | Implemented behind `otclientV8NativeEnabled` plus explicit `otclientV8NativeEmptyWorldEnabled`/thing-ID configuration; real map streaming, content loading, collision, and gameplay remain blocked. |
