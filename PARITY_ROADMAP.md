...
| P4 — 7.4 login/game | Independently specified login and game sessions, character list, encryption boundary, version-safe packet codecs, and profile-selected native client behavior. | Synthetic fixtures and authorized interoperability test harnesses. | Legacy RSA/XTEA and FE-aware foundations remain available. A separate profile-selected normal OTCv8 740 path now decodes numeric-account login/pending-game requests and, when explicitly enabled with operator-owned thing IDs, sends standard player-login/full-map frames and classic cardinal movement responses. |
| P5 — world/gameplay | Original map/entity simulation, movement, combat, conditions, NPC/monster behavior, and persistence. | Deterministic simulation tests. | Original empty-world manifest, deterministic viewport/tick, cardinal movement acknowledgement, and position persistence foundations exist for both the FE-aware custom-client path and the bounded native 740 fixture. Real maps, map deltas, entities, items, collision, combat, and gameplay remain planned. |
...
The native OTClientV8 path is distinct from that custom-client foundation. It is enabled only by a `config.lua` client profile whose behavior fields match the current original codec: protocol `740`, numeric account IDs, no login packet encryption, no protocol checksum, and no login challenge. Its normal login listener accepts a numeric account ID and emits the standard legacy character-list layout. Its normal game listener sends player-login/full-map initialization and classic cardinal movement responses only when the operator explicitly enables the empty-world fixture and provides nonzero matching ground and player-look identifiers from their own client data. The implementation does not select this behavior by testing an FE profile name, and it rejects unsupported client-profile combinations rather than silently applying 740 assumptions. Its bounded ground fixture does not claim map loading, map-row deltas, content distribution, or gameplay. The complete acceptance boundary is documented in [OTCLIENTV8_740_NATIVE_CONTRACT.md](OTCLIENTV8_740_NATIVE_CONTRACT.md).

## Compatibility safeguards

The FE command line will use TFS-style startup phases and actionable diagnostics, but it will label feature gates explicitly. A successful run can state that the configured status, legacy, custom-session, or native empty-world services are listening while still stating that real map streaming and gameplay remain unavailable. A release may not label itself broadly playable or client-compatible until P4/P5 normal-world acceptance criteria pass.
